...
class eICUReader(object):
    ...
    def __init__(self, data_path, device=None, labs_only=False, no_labs=False):
        ...
        self.flat = self.flat.drop(['hospitalid'], axis=1) if 'hospitalid' in self.flat.columns else self.flat
...
if __name__=='__main__':
    ...
    hospital_id_list = [0]
    for id in hospital_id_list:
        path = '/home/zty/Mdata/mod/'
        hosp_path = "hosp" + str(id)
        mimic_reader = MIMICReader(path + hosp_path + "/test",device=device)

        print(mimic_reader.D,mimic_reader.F,mimic_reader.no_flat_features)
